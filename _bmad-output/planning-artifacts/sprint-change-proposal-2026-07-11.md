...
### 待 PM 执行（非文件编辑）
- ~~SM-8 基线冻结（Epic 4 dev 启动前）。~~ → **已完成（2026-07-11）**：冻结为 pre-launch baseline。项目当前无真实流量（prod 一直诚实降级、无公开用户），故深度阅读占比（触达证据时间线或 AI 深读 / 时间流 DAU）与中位会话时长的基线值均记为 **N/A — 待首波真实流量后立即立基**。冻结动作本身完成、可追溯，Epic 4 dev 启动前置解除。立基义务（首次真实流量出现时记录基线数值、上线后对比是否「深度阅读不降」）转入待办，不阻 dev。
