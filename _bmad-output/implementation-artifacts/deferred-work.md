...
- source_spec: `_bmad-output/implementation-artifacts/spec-4-3-session-and-category-filters.md`
  summary: Category filter issues an unbounded `listPublishedAssociations` full-table read + in-memory `Map<hotEventId,Set<Kind>>` build on the public home hot path for every `?category=` request (no row cap, no guardrail).
  evidence: `apps/web/app/(public)/page.tsx` reads `listPublishedAssociations({ prisma, traceId })` (filter-free by design) whenever `?category=` is active, then JS-joins against the (≤50-row capped) timeline. V1 scale is tiny so this is the endorsed 2.2-pattern trade-off, but on the public hot path it is an unbounded allocator as published association volume grows. The structural fix (normalize the Json `items` column into a queryable sub-table + a `category` SQL filter/index) is the epic-level defer already recorded for Json-column queryability; this entry records the hot-path-read angle specifically.

- source_spec: `_bmad-output/implementation-artifacts/spec-4-3-session-and-category-filters.md`（4.3 follow-up review pass 登记）
...
- source_spec: `{project-root}/_bmad-output/implementation-artifacts/spec-8-8-crash-calendar-date-detail-page.md`
  summary: crash-calendar 路由族（index + `[date]`）无自动化 e2e/seed——populated-breadth 渲染路径、`narrowDragonTiger` 收窄、404/noindex 契约无回归网
  evidence: spec-8-8 step-04 四路 review（adversarial/edge-case/verification-gap/intent-alignment）收敛点：repo 对每个 DB-backed 公开路由有 `seed-*.ts` + `@tag` e2e（`pnpm --filter web e2e:<tag>`，daily/market-reaction/associations/search/timeline/follow/watchlist/detail 均备），唯独 crash-calendar（8.3–8.8）从未有。`pnpm build` 仅证编译+动态注册（AD-3 build 不触 DB），手动 curl 覆盖了 404/noindex/breadth===null/index 链接，但 **breadth!==null** 渲染（5 段、`fmtYi` 亿格式、`narrowDragonTiger` shape 收窄、涨跌比、龙虎榜 Top-N）从未被任何自动化测试触达；repo 内无任何 spec 断言 `robots`/noindex。`fmtYi` 量级翻转（/1e8→/1e4）、`narrowDragonTiger` 闸门回退、`robots` 标志翻 true、cell href 回退 `?d=`、404→静默回退 等回归都会绿进 prod。延后到独立「crash-calendar e2e+seed」story，镜像 `e2e:daily`（最近类比：DB-backed force-dynamic 公开路由，seed 驱动，serial），并在默认 `e2e` 的 `--grep-invert` 排除表加 `@crash-calendar`。
- source_spec: `{project-root}/_bmad-output/implementation-artifacts/spec-8-8-crash-calendar-date-detail-page.md`
  summary: `[date]` 详情页 list+find 受 `listPublishedCrashDays` 默认 `limit=200` 上限——>200 个已发布大跌日后，更早的已存在日期 deep-link 会假 404
  evidence: spec-8-8 step-04 adversarial #3：`listPublishedCrashDays`（core，`limit=200` default，`take:limit`）被 `[date]` 页无参调用，然后 `crashDays.find(c => formatDay(c.tradeDate) === date)`。一旦 `published_crash_days` > 200 行，第 201 行之后的更早日期 `.find()` 返回 undefined → `notFound()` → 对一个**真实存在已发布行**的日期返回 404（违反 AC3「无已发布行→404」的前提）。与 8.3 索引页 `?d=` 的 list+find+cap 范式同型（pre-existing，8.8 迁移至此）。V1 体量极小（~1 个大跌日）故当前不可达；正解 = core 加 `getPublishedCrashDay({tradeDate})`（`findUnique` by PK 单行），属本 story 明示 Never（不改 core）范围外。升级路径：deferred 到独立 core story（与 [[aguhot-core-dist-and-prisma-rebuild]] 流程一致：migrate→generate→rebuild dist→restart dev）。
