...
export async function mergeHotEvents(
  ...
  const sourceLinks = await prisma.hotEventEvidence.findMany({
  ...
  const targetExisting = await prisma.hotEventEvidence.findMany({
    where: { hotEventId: targetId },
    select: { evidenceRecordId: true },
  });
  const targetRecordIds = new Set(targetExisting.map((l) => l.evidenceRecordId));
  ...
  for (const link of sourceLinks) {
    ...
    await prisma.hotEventEvidence.delete({ where: { id: link.id } });
  }
  ...
  // evidence link chain.
  const targetMembers = await prisma.evidenceRecord.findMany({
...
export async function splitHotEvent(
  ...
  const selectedRecords = await prisma.evidenceRecord.findMany({
  ...
  const newEvent = await prisma.hotEvent.create({
    data: {
      id: newTraceId(),
      title: trimmedTitle,
  ...
  const remainingMembers = await prisma.evidenceRecord.findMany({
